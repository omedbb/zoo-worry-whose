...
- (void)setup {
  M2AppDelegate* appDelegate = (M2AppDelegate*)[[UIApplication sharedApplication] delegate];
    NSLog(@"M2SettingsViewController::setup");
  [appDelegate sendNotification:@"notifyDemoAppCrashReportReceived" callback:^{
      NSUserDefaults* userDefaults = [NSUserDefaults standardUserDefaults];
      [userDefaults setBool:YES forKey:@"has_sent_crashreport"];
      [userDefaults synchronize];
      NSLog(@"M2SettingsViewController::setup - crashing in some number of milliseconds");
      usleep(500*1000);
